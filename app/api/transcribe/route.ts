...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await ai.run('@cf/openai/whisper-large-v3-turbo', { audio: Array.from(bytes), task: 'transcribe', vad_filter: true, condition_on_previous_text: true }) as { text?: string; transcription_info?: { text?: string } };
