...
export async function POST(request: Request) {
  ...
  try {
    ...
    const seconds = typeof body.seconds === 'number' ? body.seconds : 0, category = body.category === 'training' ? 'training' : body.category === 'translation' ? 'translation' : body.category === 'summary' ? 'summary' : null;
    if (!category || seconds <= 0 || seconds > (category === 'summary' ? 86_400 : 60)) return json({ error: '计时数据无效' }, 400);
