...
import { Account, checkAdminPassword, clearAdminCookie, ensureBootstrap, hashPassword, isAdmin, PLAN_DEFAULTS, requireAdmin } from '@/lib/server/auth';
import { deepSeekConfigured, setDeepSeekKey } from '@/lib/server/config';
...
async function dashboard() {
  ...
  return { users, prices: prices.results, history: history.results, deepseekConfigured: await deepSeekConfigured(), costMultiplier: Math.max(.1, Math.min(100, Number(multiplier?.value) || 1)) };
}

export async function GET(request: Request) {
  try { if (!await isAdmin(request)) return json({ authenticated: false }, 401); return json({ authenticated: true, ...(await dashboard()) }); }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === 'login') {
      ...
      return json({ authenticated: true, ...(await dashboard()) }, 200, { 'Set-Cookie': cookie });
    }
