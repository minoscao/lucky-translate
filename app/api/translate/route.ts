...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await deepSeekJson(account, '翻译', [{ role: 'system', content: interpreterPrompt(upper.name, lower.name) }, { role: 'user', content: JSON.stringify({ previous_utterances: context, current_utterance: source }) }], request.signal, 3000, validate);
