...
export default function AdminPage() {
  ...
  const [accessConfigured, setAccessConfigured] = useState<boolean | null>(null);
  const [creatingUser, setCreatingUser] = useState(false);
  const [loading, setLoading] = useState(true), [error, setError] = useState('');
  const load = async () => { setLoading(true); try { const response = await fetch('/api/admin', { credentials: 'same-origin', cache: 'no-store' }); const next = await response.json() as AdminData & { accessConfigured?: boolean }; if (response.status === 401) { setAccessConfigured(next.accessConfigured ?? null); setData(undefined); setError(''); return; } if (!response.ok) throw new Error('暂时无法连接后台，请重试'); setData(next); setAccessConfigured(true); setError(''); } catch { setData(undefined); setError('暂时无法连接后台，请重试'); } finally { setLoading(false); } };
  ...
  if (!data?.authenticated) return <main className="admin-page"><section className="admin-login"><span><ShieldCheck /></span><h1>Lucky 管理后台</h1><p>{accessConfigured === false ? '后台认证尚未配置完成，暂时无法登录。' : '通过 Cloudflare 验证管理员身份后进入。'}</p>{error && <p role="alert">{error}</p>}{accessConfigured === true ? <Button onClick={() => { window.location.href = '/admin'; }}><ShieldCheck />通过 Cloudflare 登录</Button> : <Button disabled={loading} onClick={() => void load()}><RefreshCw />检查连接状态</Button>}</section></main>;
