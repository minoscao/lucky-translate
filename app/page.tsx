  ...
  <Dialog open={visibleDialog !== null} onOpenChange={value => { if (!value) { if (visibleDialog === 'name') saveName(ownName); else closeDialog(); t.setNeedsSettings(false); } }}>
    <DialogContent className={`app-dialog ${visibleDialog === 'history' ? 'conversation-dialog' : ''} ${visibleDialog === 'edit' ? 'edit-dialog' : ''} ${visibleDialog === 'ielts' ? 'ielts-dialog' : ''} ${layoutMode === 'face-to-face' && dialogSide === 0 && !t.needsSettings && !['history', 'edit', 'usage', 'ielts'].includes(visibleDialog || '') ? 'upside-down' : ''}`} showCloseButton={false}>
      ...
      {visibleDialog === 'settings' && <div className="settings-form">
        ...
        <div className="profile-setting"><span>{account.username} · {account.level.toUpperCase()}</span><strong title={`${balancePeriod} remaining`}><FishAmount seconds={remaining} label={balancePeriod} /></strong></div><Button variant="outline" onClick={() => open('usage')}>Fish usage</Button>
      ...
      {visibleDialog === 'usage' && <div className="usage-details">{usageRows.map(item => <article key={item.label}><span>{item.label}</span><strong>${(item.cost * account.costMultiplier).toFixed(2)}</strong><small>{item.tokens.toLocaleString()} tokens · Used <FishAmount seconds={item.seconds} /> · Actual ${item.cost.toFixed(4)}</small></article>)}<p>Display multiplier: <strong>×{account.costMultiplier.toFixed(1)}</strong>. Fish usage is based on conversation text; recaps use 10%. Playback speed does not affect usage. Tokens reflect actual model usage.</p>{formError && <p role="alert">{formError}</p>}{timeLoading ? <p role="status">Loading usage…</p> : <TimeLedger entries={timeEntries} unit="points" />}</div>}
