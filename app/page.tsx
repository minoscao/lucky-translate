  ...
  <Dialog open={visibleDialog !== null} onOpenChange={value => { if (!value) { if (visibleDialog === 'name') saveName(ownName); else closeDialog(); t.setNeedsSettings(false); } }}>
    <DialogContent className={`app-dialog ${visibleDialog === 'history' ? 'conversation-dialog' : ''} ${visibleDialog === 'edit' ? 'edit-dialog' : ''} ${visibleDialog === 'ielts' ? 'ielts-dialog' : ''} ${layoutMode === 'face-to-face' && dialogSide === 0 && !t.needsSettings && !['history', 'edit', 'usage', 'ielts'].includes(visibleDialog || '') ? 'upside-down' : ''}`} showCloseButton={false}>
      ...
      {visibleDialog === 'settings' && <div className="settings-form">
        ...
        <div className="profile-setting"><span>{account.username} · {account.level.toUpperCase()}</span><strong title={`${balancePeriod} remaining`}><FishAmount seconds={remaining} label={balancePeriod} /></strong></div><Button variant="outline" onClick={() => open('usage')}>小鱼干明细</Button>
        ...
        <details className="inline-help"><summary>小鱼干 使用规则<ChevronDown /></summary><p>翻译和 English Coach 按本次对话的文字量扣除 小鱼干；总结按对应对话的 10% 计算。空闲、等待和失败回复不扣 小鱼干，播放倍速不影响扣费。Token 按模型实际用量计入；导出不额外扣 小鱼干。</p></details>
      ...
      {visibleDialog === 'usage' && <div className="usage-details">{usageRows.map(item => <article key={item.label}><span>{item.label}</span><strong>${(item.cost * account.costMultiplier).toFixed(2)}</strong><small>{item.tokens.toLocaleString()} tokens · 已用 {formatPoints(item.seconds)} · 实际 ${item.cost.toFixed(4)}</small></article>)}<p>显示倍率 <strong>×{account.costMultiplier.toFixed(1)}</strong>。翻译和训练按双方本次对话的文字量扣除 小鱼干；总结按 10% 计算；不受播放倍速影响。Token 按模型实际用量计入。</p>{formError && <p role="alert">{formError}</p>}{timeLoading ? <p role="status">正在加载明细…</p> : <TimeLedger entries={timeEntries} unit="points" />}</div>}
