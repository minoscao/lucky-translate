...
type PanelProps = {
  ...
  onLanguage: (pair: Pair) => void; onType: () => void; onSpeak: () => void; onCopy: () => void;
};
function LanguagePanel({ controller, onHistory, usage, multiplier, side, isSelf, ownName, pair, entries, locked, canSpeak, onLanguage, onType, onSpeak, onCopy }: PanelProps) {
    ...
    <div className="conversation-body">
      ...
      <section ref={viewport} className="transcript-viewport" tabIndex={0} aria-label="对话内容，可上下滚动">
        <div className="transcript-stack" lang={pair[side]}>
          {entries.length ? entries.map((item, index) => <p key={item.id} className={`transcript-line ${index === entries.length - 1 ? 'current' : 'previous'}`} dir="auto">{item.text}</p>) : <p className="transcript-line current empty">{INTERFACE_COPY[pair[side]].ready}</p>}
        </div>
      </section>
...
export default function Home() {
  ...
  const [dialog, setDialog] = useState<'settings' | 'help' | 'history' | 'text' | 'name' | null>(null);
  ...
  return <main className="translator"><div className="app-frame">
    {([0, 1] as const).map(side => <LanguagePanel key={side} controller={t} onHistory={() => open('history')} usage={t.usage} multiplier={t.multiplier} side={side} isSelf={t.selfOnTop ? side === 0 : side === 1} ownName={ownName} pair={t.pair} entries={panelEntries[side]} locked={locked} canSpeak={canSpeak} onLanguage={t.changePair} onType={() => { setDraftText(''); open('text', side); }} onSpeak={() => speak(side)} onCopy={() => void copy(side)} />)}
  ...
  <Dialog open={visibleDialog !== null} onOpenChange={value => { if (!value) { if (visibleDialog === 'name') saveName(ownName); else setDialog(null); t.setNeedsSettings(false); } }}>
    <DialogContent className={`app-dialog ${visibleDialog === 'history' ? 'conversation-dialog' : ''} ${dialogSide === 0 && !t.needsSettings && visibleDialog !== 'history' ? 'upside-down' : ''}`} showCloseButton={false}>
      <DialogHeader><DialogTitle>{visibleDialog === 'name' ? '怎么称呼你？' : visibleDialog === 'settings' ? '连接翻译服务' : visibleDialog === 'history' ? '完整对话' : visibleDialog === 'text' ? '输入文字' : '随时打开，面对面聊'}</DialogTitle><DialogDescription>
        {visibleDialog === 'name' ? 'What should we call you?' : visibleDialog === 'settings' ? '使用你自己的服务密钥开启语音识别和翻译。' : visibleDialog === 'history' ? (locked ? '正在整理最后的对话…' : `${t.history.length} 句 · 仅保留在本次页面中`) : visibleDialog === 'text' ? '任意语言都可以，会同时转换成双方的语言。' : '添加到手机主屏幕，像应用一样打开。'}
