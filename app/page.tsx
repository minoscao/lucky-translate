...
import { isVoicePackCached } from '@/lib/voice-pack';
...
import { LocalSpeech, type VoiceState } from '@/lib/local-speech';
...
function AccountWorkspace({ initialAccount, onAccount, loadError }: { initialAccount: AccountSnapshot | null; onAccount: (account: AccountSnapshot | null) => void; loadError: boolean }) {
  ...
  const localSpeech = useRef<LocalSpeech | undefined>(undefined);
  const [voiceState, setVoiceState] = useState<VoiceState>({ state: 'idle', progress: 0 });
  ...
  useEffect(() => {
    ...
    let disposed = false;
    const engine = new LocalSpeech(state => { if (!disposed) { setVoiceState(state); if (state.state === 'error') setVoiceSetupOpen(true); } }); localSpeech.current = engine;
    void isVoicePackCached().then(cached => {
      if (disposed) return;
      if (cached) void engine.prepare().catch(() => {});
    ...
    return () => {
      disposed = true; speechAbort.current?.abort(); engine.dispose(); localSpeech.current = undefined;
      document.removeEventListener('pointerup', unlock, true);
      document.removeEventListener('touchend', unlock, true);
      document.removeEventListener('keydown', unlock, true);
    };
  }, [activeAccountId]);
  const retryVoice = () => { setVoiceSetupOpen(true); setSpeechError(''); void localSpeech.current?.unlock().catch(() => {}); void localSpeech.current?.prepare().catch(() => {}); };
  ...
  useEffect(() => {
    ...
    }).catch(error => { if (!cancelled) setFormError(error instanceof Error ? error.message : 'Could not load usage details.'); }).finally(() => { if (!cancelled) setTimeLoading(false); });
  ...
  const changePassword = async () => {
    ...
    setFormError('');
    if (nextPassword !== confirmPassword) { setFormError('The new passwords do not match.'); return; }
    ...
    catch (error) { setFormError(error instanceof Error ? error.message : 'Could not change your password. Please try again.'); }
  ...
  const playSpeech = useCallback(async (text: string, lang: string) => {
    ...
    try {
      if (appMode === 'coach') {
        if (voiceState.state !== 'ready') { setVoiceSetupOpen(true); stopSpeech(); return; }
        const engine = localSpeech.current;
        if (!engine) throw new Error('Local voice is not ready. Please try again.');
        await engine.play(speechSegments(text.replaceAll('**', '').trim()), abort.signal, speechSpeed, () => { if (requestId === speechRequest.current) setSpeechPreparing(false); });
  ...
  }, [addUsage, reportError, speechSpeed, stopSpeech, appMode, dualAudio, audioRoutes, scope, speechCache, voiceState.state]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    playedCoachSpeech.current = request.id;
    if (voiceState.state === 'ready') void playSpeech(request.text, 'en');
  }, [appMode, coach.speechRequest, playSpeech, voiceState.state]);
  useEffect(() => {
  ...
  const logout = async () => {
    ...
    catch { setFormError('Could not log out. Please try again.'); }
  };
  const install = async () => {
  ...
  const usageRows = account ? [
    { label: 'Today', tokens: account.usage.todayTokens, cost: account.usage.todayCost, seconds: account.usage.todaySeconds },
    { label: 'This month', tokens: account.usage.monthTokens, cost: account.usage.monthCost, seconds: account.usage.monthSeconds },
    { label: 'All time', tokens: account.usage.totalTokens, cost: account.usage.totalCost, seconds: account.usage.totalSeconds },
  ] : [];
  ...
  const balancePeriod = account?.limits.dailySeconds ? 'Today' : 'This month';
  ...
  <Dialog open={visibleDialog !== null} onOpenChange={value => { if (!value) { if (visibleDialog === 'name') saveName(ownName); else closeDialog(); t.setNeedsSettings(false); } }}>
    <DialogContent className={`app-dialog ${visibleDialog === 'history' ? 'conversation-dialog' : ''} ${visibleDialog === 'edit' ? 'edit-dialog' : ''} ${visibleDialog === 'ielts' ? 'ielts-dialog' : ''} ${layoutMode === 'face-to-face' && dialogSide === 0 && !t.needsSettings && !['history', 'edit', 'usage', 'ielts', 'audio'].includes(visibleDialog || '') ? 'upside-down' : ''}`} showCloseButton={false}>
      ...
      {visibleDialog === 'ielts' && <section className="ielts-picker" aria-label="IELTS level guide">
        {IELTS_BANDS.map(band => <button key={band.score} type="button" className={ieltsScore === band.score ? 'selected' : ''} aria-pressed={ieltsScore === band.score} onClick={() => saveIeltsScore(band.score)}>
          <strong><span>IELTS {band.score}</span>{ieltsScore === band.score && <Check aria-label="Selected" />}</strong><span lang="en">{band.en}</span>
        </button>)}
      </section>}
      ...
      {visibleDialog === 'settings' && <div className="settings-form">
        ...
        <details className="inline-help"><summary>How fish are used<ChevronDown /></summary><p>Translator and English Coach use fish based on the amount of conversation text. Recaps use 10% of the corresponding conversation amount. Idle time, waiting, failed replies and exports use no fish. Playback speed does not affect usage. Tokens reflect actual model usage.</p></details>
        <div className="account-storage"><span>Cloud storage</span><strong>{(account.storage.bytes / 1024 / 1024).toFixed(1)} / {(account.storage.limitBytes / 1024 / 1024).toFixed(0)} MB</strong></div>
        <Button type="button" variant="ghost" className="clear-key" onClick={() => void logout()}><LogOut />Log out</Button>
      </div>}
      ...
      {visibleDialog === 'usage' && <div className="usage-details"><UsageDashboard key={account.id} url="/api/account?dashboard=1" request={accountRequest} />{usageRows.map(item => <article key={item.label}><span>{item.label}</span><strong>${(item.cost * account.costMultiplier).toFixed(2)}</strong><small>Used <FishAmount seconds={item.seconds} /> · Actual ${item.cost.toFixed(4)}</small></article>)}<p>Display multiplier: <strong>×{account.costMultiplier.toFixed(1)}</strong>. Fish usage is based on conversation text; recaps use 10%. Playback speed does not affect usage. Tokens reflect actual model usage.</p>{formError && <p role="alert">{formError}</p>}{timeLoading ? <p role="status">Loading usage…</p> : <TimeLedger entries={timeEntries} unit="points" />}</div>}
