...
import { AccountSnapshot, saveCloudRecord } from '@/lib/account';
...
export function useCoach(_serviceKey: string, addUsage: UsageHandler, ieltsScore?: number) {
  ...
  useEffect(() => {
    ...
    void fetch('/api/cloud', { credentials: 'same-origin' }).then(async response => response.ok ? await response.json() as { records?: Array<{ id: string; data: unknown }> } : null).then(payload => {
      ...
      void fetch('/api/account', { credentials: 'same-origin' }).then(async response => response.ok ? await response.json() as { account: AccountSnapshot } : null).then(data => {
        if (data) saveJournal({ ...journalRef.current, todayDate: dateKey(), todaySeconds: data.account.usage.todayTrainingSeconds, totalSeconds: data.account.usage.totalTrainingSeconds });
      }).catch(() => {});
    });
  }, [saveJournal]);
  ...
  const createPractice = useCallback(async () => {
    ...
    const controller = new AbortController(); abort.current = controller; setBusyState(true); setError('');
    try { const result = await coachPracticeDirect({ key: keyRef.current, history: historyRef.current, memory: memoryRef.current, signal: controller.signal }); setPractice(result.data); applyUsage(result.usage); return true; }
  ...
  const summarizeToday = useCallback(async () => {
    ...
    try {
      ...
      for (const start of oldStarts) {
        ...
        const weekly = await coachWeeklySummaryDirect({ key: keyRef.current, daily: days, signal: controller.signal }); applyUsage(weekly.usage);
