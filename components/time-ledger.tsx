...
export function TimeLedger({ entries, unit = 'seconds' }: { entries: TimeEntry[]; unit?: 'seconds' | 'points' }) {
  return <section className="time-ledger"><h3>{unit === 'points' ? 'Points breakdown' : 'Time breakdown'} <small>{unit === 'points' ? '积分明细' : '扣时明细'}</small></h3>{entries.length === 0 ? <p>No charges yet. 尚无扣费记录。</p> : <ol>{entries.map(entry => <li key={entry.id}>
