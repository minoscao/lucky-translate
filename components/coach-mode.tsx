...
type Props = { coach: CoachController; speaking: boolean; ieltsScore?: number; initialStage?: 'chat' | 'dashboard'; onBack: () => void; onSettings: () => void; onIelts: () => void; onSecurity: () => void; onLogout: () => void; onSpeak: (text: string) => void };
...
export function CoachMode({ coach, speaking, ieltsScore, initialStage = 'chat', onBack, onSettings, onIelts, onSecurity, onLogout, onSpeak }: Props) {
  ...
  return <main className="coach-page"><section className="coach-shell">
    ...
    {stage === 'dashboard' && <section className="coach-dashboard"><header><div><small>PERSONAL CENTER</small><h1>Your learning</h1></div><Button variant="ghost" onClick={onSettings}><Settings2/>设置</Button></header>
      ...
      <section className="learning-history"><h2>历史记录</h2><div className="coach-stats"><article><strong>{Math.max(0, Math.round(coach.todaySeconds / 60))}</strong><span>minutes today</span></article><article><strong>{coach.dailySummaries.length}</strong><span>daily recalls</span></article><article><strong>{coach.weeklySummaries.length}</strong><span>weekly recalls</span></article></div>{coach.dailySummaries.slice(0, 3).map(item => <button key={item.id} type="button" className="history-summary" onClick={() => { setSelectedSummary(item); setStage('summary'); }}><span>{item.date} · Daily summary</span><strong>{item.minutes} min</strong></button>)}</section>
      <section className="summary-shortcuts"><h2>Summary</h2><button type="button" onClick={() => { if (latestToday) { setSelectedSummary(latestToday); setStage('summary'); } else void summarize(); }} disabled={!latestToday && (coach.history.length < 2 || coach.busy)}><Sparkles/><span><strong>今日 Summary</strong><small>{latestToday ? `${latestToday.minutes} min · 查看今日重点` : '完成一段对话后生成'}</small></span></button><button type="button" disabled={!coach.weeklySummaries[0]} onClick={() => { const report = coach.weeklySummaries[0]; if (report) exportRecall(report); }}><BookOpenText/><span><strong>上周 Summary</strong><small>{coach.weeklySummaries[0] ? `${coach.weeklySummaries[0].startDate} — ${coach.weeklySummaries[0].endDate}` : '暂时没有上周总结'}</small></span></button></section>
      <section className="center-options"><h2>账户</h2><Button variant="ghost" onClick={onSettings}><GraduationCap/>我的会员与个人资料</Button><Button variant="ghost" onClick={onSecurity}><KeyRound/>安全密码</Button><Button variant="ghost" onClick={onLogout}><ArrowLeft/>退出登录</Button></section>
    </section>}
