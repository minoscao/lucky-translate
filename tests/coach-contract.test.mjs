...
const configUrl = moduleUrl(`import {COACH_RESPONSE_CONTRACT,DEFAULT_COACH_SKILL} from '${coachUrl}'; const getDb=()=>({prepare:()=>({first:async()=>globalThis.__contractSkill})});\n${configSource}`);
const routeSource = (await read('../app/api/coach/route.ts')).replace(/^import .*;\r?\n/gm, '');
const timeUrl = moduleUrl(await read('../lib/text-time.ts'));
const routeUrl = moduleUrl(`import {getCoachSkill} from '${configUrl}'; import {coachTimeBasis,parseCoachContent} from '${timeUrl}';
