...
export async function accountSnapshot(account: Account) {
  ...
  const [today, monthUsage, totalUsage, storage, multiplierRow] = await Promise.all([
    ...
    db.prepare('SELECT COALESCE(SUM(tokens), 0) tokens, COALESCE(SUM(cost_micros), 0) cost_micros, COALESCE(SUM(active_seconds), 0) active_seconds, COALESCE(SUM(training_seconds), 0) training_seconds FROM usage_daily WHERE user_id = ?1').bind(account.id).first<{ tokens: number; cost_micros: number; active_seconds: number; training_seconds: number }>(),
