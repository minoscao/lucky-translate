...
export async function recordDeepSeekUsage(account: Account, feature: string, usage?: TokenUsage, tokenMultiplier = 1) {
  ...
  await getDb().batch([
    getDb().prepare(`INSERT INTO usage_events (id, user_id, feature, provider, model, input_tokens, cached_tokens, output_tokens, total_tokens, cost_micros, price_snapshot, created_at)
      VALUES (?1, ?2, ?3, 'deepseek', 'deepseek-v4-flash', ?4, ?5, ?6, ?7, ?8, ?9, ?10)`).bind(crypto.randomUUID(), account.id, feature, input, cached, output, chargedTokens, costMicros, JSON.stringify({ ...rates, actualTokens: tokens, tokenMultiplier: multiplier }), now),
  ...
  return { tokens: chargedTokens, actualTokens: tokens, tokenMultiplier: multiplier, cost, inputTokens: input, cachedTokens: cached, outputTokens: output, period: rates.period };
}
