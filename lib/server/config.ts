...
export async function setDeepSeekKey(value: string) {
  ...
  const payload = `v2.${b64(iv)}.${b64(new Uint8Array(encrypted))}`, now = Date.now();
  await getDb().prepare(`INSERT INTO app_config (key, value, updated_at) VALUES ('deepseek_api_key_v2', ?1, ?2)
...
export async function getDeepSeekKey() {
  const row = await getDb().prepare("SELECT value FROM app_config WHERE key = 'deepseek_api_key_v2'").first<{ value: string }>();
  if (!row?.value) throw Object.assign(new Error('管理员尚未配置 DeepSeek 服务'), { status: 503 });
  const [version, rawIv, rawData] = row.value.split('.'); if (version !== 'v2' || !rawIv || !rawData) throw new Error('DeepSeek 配置无效');
...
export async function deepSeekConfigured() {
  return Boolean(await getDb().prepare("SELECT 1 ok FROM app_config WHERE key = 'deepseek_api_key_v2'").first());
}
